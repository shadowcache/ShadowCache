 ...
 */
public class MultipleBloomShadowCacheManager implements ShadowCache {
  private final int mNumBloomFilter;
  private final long mPerBloomFilterMemoryOverhead;
   ...
   */
  public MultipleBloomShadowCacheManager(ShadowCacheParameters conf) {
    long windowMs = conf.mWindowSize;
    mNumBloomFilter = conf.mNumBloom;
    // include the 1 extra working set bloom filter
    mPerBloomFilterMemoryOverhead =
        FormatUtils.parseSpaceSize(conf.mMemoryBudget) * 8 / (mNumBloomFilter + 1);
    // assume 3% Guava default false positive ratio
    mBloomFilterExpectedInsertions =
        (long) ((-mPerBloomFilterMemoryOverhead * Math.log(2) * Math.log(2)) / Math.log(0.03));
  ...
  public long getSpaceBits() {
    return mPerBloomFilterMemoryOverhead * (mNumBloomFilter + 1);
  }

  @Override
  public String getSummary() {
    return "MultipleBloomShadowCache:" + "\nnumBloomFilter: " + mNumBloomFilter + "\nmemory(MB): "
